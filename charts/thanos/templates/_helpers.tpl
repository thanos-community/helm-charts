...
{{- $hu := $root.Values.nonExistentKey -}}

{{- if not (kindIs "invalid" $comp.hostUsers) -}}
  {{- $hu = $comp.hostUsers -}}
{{- else if not (kindIs "invalid" $par.hostUsers) -}}
  {{- $hu = $par.hostUsers -}}
{{- else if hasKey $root.Values "global" -}}
  {{- if not (kindIs "invalid" $root.Values.global.hostUsers) -}}
    {{- $hu = $root.Values.global.hostUsers -}}
  {{- end -}}
{{- end -}}

{{- if not (kindIs "invalid" $hu) }}
hostUsers: {{ $hu }}
{{- end }}
{{- end }}
...
The hashring is resolved from this block for both workloads, so a hashring
configured under `receive.router` would silently do nothing. Reject it.
*/ -}}
{{- define "thanos.receive.cfg" -}}
{{- if and .Values.receive.enabled (hasKey (.Values.receive.router | default dict) "hashrings") -}}
{{- fail "receive.router.hashrings is not read: configure the hashring under receive.ingester.hashrings in split mode, or receive.hashrings in standalone mode" -}}
{{- end -}}
